#This is a analysis of Barrack Obama's state of the union address

import re

presidentalUnionString: str = """Mr. Speaker, Mr. Vice President, Members of Congress, my fellow Americans:
...
modifiedPresidentialUnionSpeech: list = presidentalUnionString.split()

def getWordCount(unionSpeech: list) -> int:
    wordCount: int = 0
    
    for word in unionSpeech:
        
        wordCount += 1
        
    return wordCount

wordCount: int = getWordCount(modifiedPresidentialUnionSpeech)

print(f"The word count for the union speech is {wordCount}")

def getCharacterCount(unionSpeech: list) -> int:
...
characterCount: int = getCharacterCount(modifiedPresidentialUnionSpeech)

print(f"The character count for the union speech is {characterCount}")

def getAverageWordLength(characterCount: int, wordCount: int) -> float:
    averageWordLength: float = characterCount / wordCount
    return averageWordLength

averageWordLength: float = getAverageWordLength(characterCount, wordCount)
print(f"The average word length is {averageWordLength: .2f} characters long")

def getAverageSentenceLength(unionString: str) -> float:
    sentenceLengthList: list = []
    #This is very hard to properly moderate, as there are meticulous edge cases that would require other complex libraries to resolve (like in Natural Language Processing)
    
    sentenceOffset: int = -2 # this is here because there are two instances of there being "Mr. " in it. There may be other unaccounted cases affecting this offset 
   
    sentenceUnionString = re.sub(r"([.!?])", r"\1 ", unionString) # adds exactly one space after ending punctuation
    sentenceSplitString: list = re.split(r"[.!?]", sentenceUnionString)
    
    sentenceLength = len(sentenceSplitString) + sentenceOffset # gets the total amount of sentences
    
    for sentence in sentenceSplitString:
        sentenceWordLength = len(sentence.split()) # gets all of the words within the individual sentence
        sentenceLengthList.append(sentenceWordLength)
        
    totalSentenceLenth: int = sum(sentenceLengthList) # gets all words across every sentence and adds them
    averageSentenceLength = totalSentenceLenth / sentenceLength # divided by total word count
...
averageSentenceLength = getAverageSentenceLength(presidentalUnionString)

print(f"The average sentence length is {averageSentenceLength: .2f}\n words")

def getDistributedWords(unionString: str) -> dict:
    wordDictonary = {}
    unionStringSplit = unionString.split()
    
    for word in unionStringSplit:
...
wordDictonary = getDistributedWords(presidentalUnionString)

def findWordsEndingInPly(unionString: str) -> list:
    unionStringSplit = unionString.split()
    wordsEndingInly = []
    
    for word in unionStringSplit:
        if word.endswith("ly"):
            wordsEndingInly.append(word)
    
    if len(wordsEndingInly) == 0:
        print("there's nothing in here")
        
    return wordsEndingInly

wordsEndingInlyTable = findWordsEndingInPly(presidentalUnionString)

#print(f'{wordsEndingInlyTable}\n ends with the suffix ly')

def top10OccuringWords(wordsDictonary: dict) -> list:
    sortedDictonary = list(sorted(wordsDictonary.items(), key = lambda word: -word[1]))
    #print(sortedDictonary)
    
    top10Table: list = []
    
    for index in range(10):
        top10Table.append(sortedDictonary[index])
    
    print(top10Table)
    return top10Table
        

top10Table: list = top10OccuringWords(wordDictonary)

print("The top 10 words are " + str(top10Table))